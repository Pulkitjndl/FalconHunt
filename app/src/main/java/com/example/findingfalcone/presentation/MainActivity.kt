...
class MainActivity : AppCompatActivity() {
    ...
    private fun renderView(state: ViewState) {
        ...
        state.planetVehicle[planets[0]]?.let { planet1Image.setImageResource(it.getImage()) }

        planet2Layout.visibility = View.VISIBLE
        planet2Name.text = planets[1].name
        state.planetVehicle[planets[1]]?.let { planet2Image.setImageResource(it.getImage()) }

        planet3Layout.visibility = View.VISIBLE
        planet3Name.text = planets[2].name
        state.planetVehicle[planets[2]]?.let { planet3Image.setImageResource(it.getImage()) }

        planet4Layout.visibility = View.VISIBLE
        planet4Name.text = planets[3].name
        state.planetVehicle[planets[3]]?.let { planet4Image.setImageResource(it.getImage()) }

        planet5Layout.visibility = View.VISIBLE
        planet5Name.text = planets[4].name
        state.planetVehicle[planets[4]]?.let { planet5Image.setImageResource(it.getImage()) }

        planet6Layout.visibility = View.VISIBLE
        planet6Name.text = planets[5].name
        state.planetVehicle[planets[5]]?.let { planet6Image.setImageResource(it.getImage()) }
    }
